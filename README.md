...
- **可验证交付**：102 项自动化测试、真实本地 MySQL/Redis 联调、无密钥 GitHub Actions 和公开离线评测基线。
...
> 设计并实现基于 FastAPI、LangGraph、Chroma、MySQL 与 Redis 的知识库问答系统，支持 normal/deep 双模式 SSE 流式回答；修复跨存储状态一致性与并行 Session 问题，引入 Bearer 单用户安全边界、流式上传限制和低敏感 turn 级可观测性，并以 102 项自动化测试、无密钥 CI 与可复现离线评测固化工程证据。
